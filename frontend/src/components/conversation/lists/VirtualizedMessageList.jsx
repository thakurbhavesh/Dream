...
const VirtualizedMessageList = ({
  ...
  const onLoadPreviousWithToast = useCallback(async () => {
    ...
    if (loaded) {
      setOlderToast("Older messages loaded");
    } else {
  ...
  return (
    <Box sx={{ position: "relative", height: "100%" }}>
      ...
      {/* Floating "Load older" button — always visible at top center when hasMore */}
      <Fade in={!searchFiltersActive && !loadingOlder && hasMore}>
        <IconButton
          size="small"
          onClick={() => { onLoadPreviousWithToast(); }}
          sx={{
            position: "absolute",
            top: 12,
            left: "50%",
